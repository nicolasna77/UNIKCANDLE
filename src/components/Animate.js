...
export function animate() {
  ...
  //------------------------------
  //------ candle -------------
  LIGHT.candleSpotLightAni.position.x =
    Math.sin(time * Math.PI * 0.3) * 0.005 +
    GVARS.CheraghPosition.x +
    GVARS.FlamePositionOffset.x;
  LIGHT.candleSpotLightAni.position.y =
    Math.cos(time * Math.PI * 0.3 + Math.random() * 0.1) * 0.005 +
    GVARS.CheraghPosition.y +
    GVARS.FlamePositionOffset.y;
  LIGHT.candleSpotLightAni.position.z =
    Math.cos(time * Math.PI * 0.3) * 0.005 +
    GVARS.CheraghPosition.z -
    +GVARS.FlamePositionOffset.z;
  LIGHT.candleSpotLightAni.intensity =
    (2 + Math.sin(time * Math.PI * 2) * Math.cos(time * Math.PI * 1.5)) * 0.1 +
